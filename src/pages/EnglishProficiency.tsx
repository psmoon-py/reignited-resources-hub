...
import { ArrowLeft, ArrowRight, CheckCircle, Lock, FileText, BookOpen, List, GraduationCap, YoutubeIcon } from "lucide-react";
import Navbar from "@/components/layout/Navbar";
import Footer from "@/components/layout/Footer";
import { englishProficiencyData } from "@/data/englishProficiencyContent";
...
const EnglishProficiency: React.FC = () => {
  const { sectionId } = useParams<{ sectionId: string }>();
  const [activeSection, setActiveSection] = useState(
    sectionId || englishProficiencyData[0].id
  );
  ...
  const currentSection = englishProficiencyData.find(section => section.id === activeSection) || englishProficiencyData[0];

  const currentIndex = englishProficiencyData.findIndex(section => section.id === activeSection);
  const prevSection = currentIndex > 0 ? englishProficiencyData[currentIndex - 1] : null;
  const nextSection = currentIndex < englishProficiencyData.length - 1 ? englishProficiencyData[currentIndex + 1] : null;

  const navigateToSection = (sectionId: string) => {
    navigate(`/english-proficiency/${sectionId}`);
  };
  ...
  const handleAuthRequired = () => {
    if (currentSection.requiresAuth && !isAuthenticated) {
      ...
      navigate("/auth", { state: { returnUrl: `/english-proficiency/${activeSection}` } });
      return true;
    }
    return false;
  };
  ...
  const getResourceTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'tool':
        return <div className="p-1 rounded-full bg-green-100 text-green-600"><BookOpen className="h-4 w-4" /></div>;
      case 'website':
      ...
      case 'app':
        return <div className="p-1 rounded-full bg-teal-100 text-teal-600"><FileText className="h-4 w-4" /></div>;
      case 'pdf':
        return <div className="p-1 rounded-full bg-pink-100 text-pink-600"><FileText className="h-4 w-4" /></div>;
      case 'book':
        return <div className="p-1 rounded-full bg-amber-100 text-amber-600"><BookOpen className="h-4 w-4" /></div>;
      case 'service':
      ...
      <main className="flex-grow py-12">
        <div className="container mx-auto px-4">
          <motion.div
            ...
            className="mb-12"
          >
            <h1 className="text-4xl font-bold mb-4">English Proficiency Tests</h1>
            <p className="text-lg text-muted-foreground mb-6">
              Comprehensive resources to help you succeed on TOEFL, IELTS, and other English proficiency exams.
            </p>
            ...
            <div className="mb-8">
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm font-medium">
                  {isAuthenticated ? `${Math.round(50)}%` : "Sign in to track progress"}
                </span>
              </div>
              <Progress value={isAuthenticated ? 50 : 0} className="h-2" />
            </div>
          </motion.div>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24">
                ...
                <ul className="space-y-2">
                  {englishProficiencyData.map((section) => (
                    <li key={section.id}>
                      <button
              ...
              <Card className="p-6">
                {currentSection.requiresAuth && !isAuthenticated ? (
                  <div className="text-center py-12">
                    ...
                    <Button
                      onClick={() => navigate("/auth", { state: { returnUrl: `/english-proficiency/${activeSection}` } })}
                    >
                      Sign in or Register
                    </Button>
                  </div>
